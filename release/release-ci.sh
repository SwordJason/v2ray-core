...
curl -L -o /v2/build/releases https://api.github.com/repos/SwordJason/v2ray-core/releases
...
go get -insecure -t github.com/SwordJason/v2ray-core/...
go get -insecure -t v2ray.com/ext/...

pushd $GOPATH/src/github.com/SwordJason/v2ray-core/
...
RELEASE_ID=$(curl --data "${JSON_DATA}" -H "Authorization: token ${GITHUB_TOKEN}" -X POST https://api.github.com/repos/SwordJason/v2ray-core/releases | jq ".id")
...
function uploadfile() {
  ...
  sleep 1
  curl -H "Authorization: token ${GITHUB_TOKEN}" -H "Content-Type: ${CTYPE}" --data-binary @$FILE "https://uploads.github.com/repos/SwordJason/v2ray-core/releases/${RELEASE_ID}/assets?name=$(basename $FILE)"
  sleep 1
}
...
ART_ROOT=$GOPATH/src/github.com/SwordJason/v2ray-core/bazel-bin/release
...
DOWNLOAD_URL="https://github.com/SwordJason/v2ray-core/releases/download/v${VERN}/v2ray-macos.zip"
...
cp "$GOPATH/src/github.com/SwordJason/v2ray-core/release/install-release.sh" ./install.sh
